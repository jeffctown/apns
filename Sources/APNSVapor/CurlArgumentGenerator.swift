...
struct CurlArgumentGenerator {
    ...
    func generate(data: Data, device: Device, token: String, certificate: APNSVapor.Certificate) throws -> [String] {
        ...
        let arguments = ["-d", "\(jsonString)", "-H", "apns-topic:\(certificate.bundleIdentifier)","-H","apns-push-type:\(device.pushType.string)", "-H", "Content-Type:application/json", "--http2", "--cert", "\(certificate.path):\(certificate.password)", remoteURL + token]
