...
public final class Device: Codable, Equatable {
    ...
    public static func == (lhs: Device, rhs: Device) -> Bool {
        return lhs.id == rhs.id && lhs.type == rhs.type && lhs.token == rhs.token && lhs.environment
            == rhs.environment && lhs.bundleIdentifier == rhs.bundleIdentifier
    }
    ...
    public init(type: String, token: String, bundleIdentifier: String, environment: APNS.Environment = .release) {
        self.type = type
        self.token = token
